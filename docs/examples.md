...
One chain, end to end — a real shape, not a toy: the **dogadopt** product, from strategic
outcome down to a single picked-up issue, plus one inbound item hitting the front door. Every
artefact below follows the live templates; the native fields sit in the comment above each
body, exactly as the [skills](skills/index.md) set them.
...
> each other. Adopters give up; dogs wait longer. One place to search every rescue changes
> both.
>
> **## Key Results**
>
...
acceptance criteria listing the observable behaviours, `product/dogadopt` inherited. Sits in
`Todo`, refined.

**A `type/bug`** — *"Refresh job drops dogs with no photo"*: steps to reproduce, expected vs
...
> **## When is it done?**
>
> Every consented rescue gets the nightly mail; a bounce or 14-day silence flags the rescue
> as stale in the coverage report.
>
> **## Context**
>
> Rescues asked for email over a portal login — meet them where they are.
>
> **## Plan**
>
...
## 4 · One inbound item — the front door working

An email lands: *"Your site shows a dog we rehomed last week."* Whoever's on
...
Had it been an ad-hoc data ask ("how many collies are listed in Powys?") it would stay
**flow work**: `flow/query`, no project, decided within 2 working days.

---

## What the example shows

The **ladder**, intact: the nightly-confirm action serves the coverage KR's freshness bar,
the project promises that KR 0% → 100%, the initiative measures why anyone cares. Ask "why am
I doing this?" at any rung and the answer is one hop up.
