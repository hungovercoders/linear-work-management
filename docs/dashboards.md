...
The model only works if drift is **visible without going looking**. These shared Linear
...
Empty views are the goal; anything in one is a conversation waiting to happen.
...
- Views are **shared, workspace-level** — favourite them into your sidebar; grouping and
  ordering are display settings, so set them once to your taste.
