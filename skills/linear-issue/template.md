<!-- The issue DESCRIPTION only. Set these as native Linear fields, NOT in this text:
     ...
     can act on. Emphasis by type:
       ...
       • chore   → the upkeep and why now
       • spike   → the question to answer and the time box -->

## Problem
<what needs doing and why — clear enough to start without asking. For a defect: steps to
...
## Agent plan
<!-- The body above is the prompt. Store the plan for HOW here (or as a comment) once worked
     out, so it's reviewable before the code is. Leave empty until then. -->
